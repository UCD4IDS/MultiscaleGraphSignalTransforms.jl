...
* `levlengths::Vector{UInt8}`: the integer sequence that specifies the length of each basis block in `levlist` (optional)
...
type BasisSpec
    ...
    function BasisSpec(levlist::Vector{UInt8};
        ...
        if Base.length(levlengths) != Base.length(levlist) && Base.length(levlengths) != 0
            ...
            new( levlist, Vector{UInt8}(0), c2f, description )
        else

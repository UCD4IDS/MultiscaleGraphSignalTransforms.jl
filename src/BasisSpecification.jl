...
* `levlist::Vector{Int}`: the integer sequence that specifies a particular basis
* `levlengths::Vector{Int}`: the integer sequence that specifies the length of each basis block in `levlist` (optional)

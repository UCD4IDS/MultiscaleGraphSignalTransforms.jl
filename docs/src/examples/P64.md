...
plot(G.f; c = :black, lw = 2, legend = false, grid = false, size = (408, 150))
xticks!([1; 8:8:64], vcat(string("1"), [string(k) for k in 8:8:64]))
```
...
for i in 1:10
    w, _ = HGLET_Synthesis(reshape(spike(important_idx[i], N), (N, 1)),
        GP, BS_lphglet, G, method = :L)
    lphglet_top10[:, i] = w[:]
end
wiggle(lphglet_top10; sc = 0.75)
p2 = title!("Top 10 LP-HGLET basis vectors")

plot(p1, p2, layout = Plots.grid(2, 1), size = (408, 300))
xticks!([1; 8:8:64], vcat(string("1"), [string(k) for k in 8:8:64]))
yticks!([0; 1:10], vcat(string(""), [string(k) for k in 1:10]))
```
...
wiggle(eghwt_top10; sc = 0.75)
p5 = title!("Top 10 eGHWT best basis vectors")

# display the top 10 basis vectors
plot(p1, p2, p3, p4, p5, layout = Plots.grid(5, 1), size = (408, 750))
xticks!([1; 8:8:64], vcat(string("1"), [string(k) for k in 8:8:64]))
yticks!([0; 1:10], vcat(string(""), [string(k) for k in 1:10]))
```
...
wiggle(wav_lp_top10; sc = 0.75)
p3 = title!("Top 10 LP-NGWP basis vectors")

plot(p1, p2, p3, layout = Plots.grid(3, 1), size = (408, 450))
xticks!([1; 8:8:64], vcat(string("1"), [string(k) for k in 8:8:64]))
yticks!([0; 1:10], vcat(string(""), [string(k) for k in 1:10]))
```
